	...
	<div>
		...
		<b-modal id="mast-file-list" v-model="modalOpenned" title="Master Schedule Files" hide-footer>
			...
			<b-list-group class="overflow-auto mh-300">
				<b-list-group-item button v-for="file in allMastFiles" @click="openMast(file)" :key="file.id" class="d-flex justify-content-between">
					<span style="width: 50%">
						<i class="fas fa-file-alt"></i>
						{{ file.title }}
					</span>
					<small style="font-style: italic;">{{ file.created_date | formatDate }}</small>
					<a href="#" @click.stop.prevent="deleteFile(file)" class="text-danger px-1" style="text-align:right;">
